...
void Ragebot::CreateMove(CUserCmd * cmd, bool& bSendPacket)
{
    ...
    if (g_Config.GetBool("rbot_onkey") && !InputSys::Get().IsKeyDown(g_Config.GetInt("rbot_key")))
        return;
    ...
    if (g_Config.GetBool("doubletap_enable"))
    {
        ...
        if (!InputSys::Get().IsKeyDown(g_Config.GetInt("doubletap_key")))
        {
            int doubletapTickcountDelta = g_GlobalVars->tickcount - lastDoubleTapInTickcount;

            if (doubletapTickcountDelta >= TIME_TO_TICKS(0.2f) && cmd->buttons & IN_ATTACK) {
...
void Ragebot::OnFireEvent(IGameEvent * event)
{
    if (!g_LocalPlayer)
        return;
...
bool Ragebot::InFakeLag(C_BasePlayer * player)
{
    bool rBool = true;
...
void Ragebot::AutoStop(CUserCmd * cmd)
{
    cmd->forwardmove = 0;
    cmd->sidemove = 0;
    cmd->upmove = 0;
...
void Ragebot::AutoCrouch(CUserCmd * cmd)
{
    cmd->buttons |= IN_DUCK;
}

void Ragebot::ZeusBot(CUserCmd * cmd, C_BaseCombatWeapon * weapon)
{
    cmd->buttons &= ~IN_ATTACK;
...
bool Ragebot::HitChance(QAngle angles, C_BasePlayer * ent, float chance)
{
...
int Ragebot::FindBestEntity(CUserCmd * cmd, C_BaseCombatWeapon * weapon, Vector & hitpos, bool& bBacktrack)
{
...
bool Ragebot::GetBestHitboxPoint(C_BasePlayer * entity, float& damage, Vector & hitbox, BaimMode baim, bool& WillKill, matrix3x4_t matrix[MAXSTUDIOBONES], mstudiohitboxset_t * StudioSet, bool NoPointscale)
{
...
void Ragebot::PrecacheShit() //need fix
{
    for (int i = 1; i < g_EngineClient->GetMaxClients(); i++)
    {
        auto entity = static_cast<C_BasePlayer*> (g_EntityList->GetClientEntity(i));
...
std::vector<PointScanStruct> Ragebot::GetPointsForScan(C_BasePlayer * entity, int hitbox, mstudiohitboxset_t * hitset, matrix3x4_t matrix[MAXSTUDIOBONES], float pointscale)
{
