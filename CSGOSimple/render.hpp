...
public:
	...
	inline void RenderBoxByType(T x1, T y1, T x2, T y2, Color color, float thickness = 1.f, int type = 0) {
		if (type == 0)
			...
			DrawCornerESP(x1, y1, x2, y2, color, thickness);
	}
	...
	template <class T>
	inline void Render3DBox(T x1, T y1, T x2, T y2, Color color, float th = 1.f) {
		int w = x2 - x1;
		int h = y2 - y1;
	...
	template <class T>
	void DrawCornerESP(T x1, T y1, T x2, T y2, Color color, float th = 1.f) {
		int w = x2 - x1;
		int h = y2 - y1;
		...
		//inline
		RenderLine(x1, y1, x1, y1 + ih, color, th);//top left
		RenderLine(x1, y1, x1 + iw, y1, color, th);
		RenderLine(x1 + w - iw, y1, x1 + w, y1, color, th); //top right
		RenderLine(x1 + w, y1, x1 + w, y1 + ih, color, th);
		RenderLine(x1, y1 + h - ih, x1, y1 + h, color, th); //bot left
		RenderLine(x1, y1 + h, x1 + iw, y1 + h, color, th);
		RenderLine(x1 + w - iw, y1 + h, x1 + w, y1 + h, color, th);//bot right
		RenderLine(x1 + w, y1 + h - ih, x1 + w, y1 + h, color, th);
	...
	template <class T>
	inline void DrawOctagonCornerESP(T x1, T y1, T x2, T y2, Color color, float th = 1.f)
	{
		int w = x2 - x1;
		int h = y2 - y1;
		...
		RenderLine(x1, y1, x1 + w, y1, color, th); //top line
		RenderLine((x1 + w) + iw, y1 + ih, x1 + w, y1, color, th); //top right
		RenderLine(x1 - iw, y1 + ih, x1, y1, color, th); // top left

		RenderLine(x1, y1 + h, x1 + w, y1 + h, color, th); //bottom line
		RenderLine((x1 + w) + iw, (y1 + h) - ih, x1 + w, y1 + h, color, th); //bottom right
		RenderLine(x1 - iw, (y1 + h) - ih, x1, y1 + h, color, th); // bottom left
	}
