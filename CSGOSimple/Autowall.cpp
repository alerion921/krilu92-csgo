...
bool Autowall::FireBullet(C_BaseCombatWeapon* pWeapon, Vector& direction, float& currentDamage)
{
    ...
    while (possibleHitsRemaining > 0 && currentDamage >= 1.f)
    {
        ...
        if (canDoDamage && static_cast<C_BasePlayer*>(enterTrace.hit_entity)->IsEnemy())
        {
...
bool Autowall::CanWallbang(float& dmg)
{
    ...
    FireBulletData data = FireBulletData(g_LocalPlayer->GetEyePos());
    data.filter = CTraceFilter();
    ...
    Math::AngleVectors1337(EyeAng, &forward);
    dst = data.src + (forward * 8196.f);

    QAngle angles;
    Math::calculate_angle1337(data.src, dst, angles);
    Math::AngleVectorsAuto(angles, &data.direction);
    ...
    TraceLine(data.src, end, MASK_SHOT | CONTENTS_GRATE, g_LocalPlayer, &data.enter_trace);
    ...
    if (HandleBulletPenetration(weapondata, data.enter_trace, g_LocalPlayer->GetEyePos(), data.direction, data.penetrate_count, data.current_damage, weapondata->flPenetration, sv_penetration_type, ff_damage_reduction_bullets, ff_damage_bullet_penetration))
    {
