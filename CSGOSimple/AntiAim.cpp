...
void AntiAim::LbyBreakerPrediction(CUserCmd* cmd, bool& bSendPacket)
{
    ...
    */
    ...
    if (!g_ClientState->m_nChokedCommands)
    {
...
void AntiAim::Pitch(CUserCmd* cmd)
{
    ...
    {
        float pitch = fmodf(g_GlobalVars->tickcount * g_Config.GetFloat("rbot_aa_spinbot_speed"), 179.f);
    ...
    {
        float pitch = -fmodf(g_GlobalVars->tickcount * g_Config.GetFloat("rbot_aa_spinbot_speed"), 89.f);
        Math::NormalizePitch(pitch);
        cmd->viewangles.pitch = pitch;
        break;
    }
    ...
    case PitchAntiAims::RANDOM:
        cmd->viewangles.pitch = Math::RandomFloat(-89.f, 89.f);
        break;
...
void AntiAim::Yaw(CUserCmd* cmd, bool fake)
{
    ...
    case YawAntiAims::RANDOM:
        cmd->viewangles.yaw = Math::RandomFloat(-179.f, 179.f);
        break;
